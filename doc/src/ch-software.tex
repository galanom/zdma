...
\begin{algorithm}
\begin{algorithmic}[1]
\Procedure{SlotReserve}{Client, AcceleratorSlots, Config}
...
\Repeat
	...
	\ForAll {Slot \in AcceleratorSlots}
		\If {Slot.Core = Client.Core}	\Comment{slot is already programmed}
			\State CoreMask |= Slot.id\;
		\EndIf
	\EndFor

	\State SelectedSlot \gets SelectSlot(CoreMask
	\Statex {\bf and} Client.EffectiveAffinity) \;
...
\Until {SelectedSlot \neq ~ \emptyset}\;
\State
\State SelectedSlot.AccessAge \gets ~ 0\;	\Comment{reset slot age on every access}
