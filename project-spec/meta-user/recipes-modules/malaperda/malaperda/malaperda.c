...
static int test_transfer(void)
{
	 ...
	 * any errors
	 */
	tx.chan_desc = dmaengine_prep_slave_single(tx.chan, tx.buf, tx.size, 
		DMA_MEM_TO_DEV, DMA_CTRL_ACK|DMA_PREP_INTERRUPT);
	if (tx.chan_desc == NULL) {
		pr_err("dmaengine_prep_slave_single() returned NULL!\n");
		return tx.cookie = -EBUSY;
	}
	rx.chan_desc = dmaengine_prep_slave_single(rx.chan, rx.buf, rx.size, 
...
static int __init malaperda_init(void)
{
	...
	tx.buf = (dma_addr_t)kmalloc(length, GFP_KERNEL);
	rx.buf = (dma_addr_t)kmalloc(length, GFP_KERNEL);
	if (!tx.buf || !rx.buf ) {
