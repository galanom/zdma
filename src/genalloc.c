...
/**
 * gen_chunk_virt_to_phys - return the physical address of memory
 * @chunk: chunk where memory is allocated
 * @addr: starting address of memory
 *
 * Returns the physical address on success, or -1 on error.
 */
/*phys_addr_t gen_chunk_virt_to_phys(struct gen_pool_chunk *chunk, unsigned long addr)
{
	phys_addr_t paddr = -1;
...
EXPORT_SYMBOL(gen_chunk_virt_to_phys);
*/
 ...
 * @limit:	skip chunks with >=limit free space
 *
 * Finds the chunk that has the maximum available memory that is strictly less than
 * the given limit.
 */
