/*  malaperda.c - The simplest kernel module.
...
MODULE_AUTHOR
    ("Xilinx Inc.");
MODULE_DESCRIPTION
    ("malaperda - loadable module template generated by petalinux-create -t modules");
...
static irqreturn_t malaperda_irq(int irq, void *lp)
{
	printk("malaperda interrupt\n");
	return IRQ_HANDLED;
}
...
static int malaperda_probe(struct platform_device *pdev)
{
	...
	struct malaperda_local *lp = NULL;
	...
	lp = (struct malaperda_local *) kmalloc(sizeof(struct malaperda_local), GFP_KERNEL);
	if (!lp) {
		dev_err(dev, "Cound not allocate malaperda device\n");
		return -ENOMEM;
	}
	...
	if (!lp->base_addr) {
		dev_err(dev, "malaperda: Could not allocate iomem\n");
		rc = -EIO;
		goto error2;
	}
	...
	if (!r_irq) {
		...
		dev_info(dev, "malaperda at 0x%08x mapped to 0x%08x\n",
			(unsigned int __force)lp->mem_start,
			(unsigned int __force)lp->base_addr);
		return 0;
	}
	lp->irq = r_irq->start;
	rc = request_irq(lp->irq, &malaperda_irq, 0, DRIVER_NAME, lp);
	if (rc) {
	...
	dev_info(dev,"malaperda at 0x%08x mapped to 0x%08x, irq=%d\n",
		(unsigned int __force)lp->mem_start,
		(unsigned int __force)lp->base_addr,
		lp->irq);
	return 0;
